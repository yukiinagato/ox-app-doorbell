...
- ビルド: `xcodebuild -project ios/Doorbell.xcodeproj -scheme Doorbell -sdk iphoneos`
  ...
  要るだけ。SIP を使うなら先に一度 `tools/build_pjsip_ios.sh` を実行しておく
  (無ければ SIP 無しでビルドされる — 呼出/通知/映像/返信は動く)。
...
初回起動で `Documents/boot.json` に既定が生成される。編集手段 (どれでも可):
...
- **管理 webui**: 既に mesh に居る別ノードの `http://<ip>:47180/admin/` → デバイス →
  参加 PIN で本端末を招き入れる (psk はこの経路で安全に配送される)。
- 手で書く場合の形式 (WPF/Android と同一):
