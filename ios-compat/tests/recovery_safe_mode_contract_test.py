...
class RecoverySafeModeContracts(unittest.TestCase):
    ...
    def test_answer_lifecycle_excludes_monitor_and_yields_losing_dialog(self):
        ...
        self.assertIn("callLifecycle.confirmAnswer(ownerIsLocal: true)", modern)
        self.assertIn("callLifecycle.confirmAnswer(ownerIsLocal: false)", modern)
        self.assertIn("callLifecycle.ended", modern)
    ...
    def test_modern_ios_media_caps_and_call_expiry_are_measured(self):
        ...
        # Decode is measured too: true only after H264SampleLayerPlayer has shown a frame.
        self.assertIn('"h264_decode": h264DecodeState == "verified" && !safeMode', capabilities)
        supervisor = read("ios/Doorbell/RuntimeSupervisor.swift")
        self.assertIn("func recordH264Decode(verified: Bool, state: String)", supervisor)
        player = read("ios/Doorbell/AdaptiveH264MjpegPlayer.swift")
        self.assertIn('Self.onDecodeState?(true, "verified")', player)
        self.assertNotIn("AVPlayer(", player)
