...
> English (this page) / 日本語: [Decisions-ja](Decisions-ja) / 中文: [Decisions-zh](Decisions-zh)

Major design decisions, recorded as "context → options → decision → rationale".
For philosophy-level discussion see [Design Philosophy](Design-Philosophy); for implementation see [Architecture](Architecture).
...
- **Decision**: (c). MJPEG (`/stream.mjpeg`) is the compatibility baseline, while only a device
  whose measured hardware path is healthy may publish HW-encoded fMP4 (`/stream.mp4`). A client
  claims either path only after its exact renderer/source has been commissioned. The iOS 5 shell
  directly plays HTTP(S) MJPEG/snapshot; that JPEG remains local and is not forwarded into Core.
- **Rationale**: MJPEG is viable on older CPUs, but compatibility is still an implemented and
  tested client property, not a universal promise. H.264 can avoid HA transcoding with go2rtc
  `#video=copy` on qualified hardware. `auto` falls back to MJPEG when codec health fails; Android
  API 19 currently has zero formally supported SKUs.
...
- **Rationale**: hand-syncing three languages across multiple platforms is guaranteed to break down. Generated files are tracked with do-not-edit headers, and duplicate or missing keys surface in CI (the repair commit is in the development history — see git log, not [FAQ](FAQ)).
