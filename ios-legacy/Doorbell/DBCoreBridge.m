...
static NSString *DBDefaultGatewayIPv4(void) {
  ...
  if (getifaddrs(&head) == 0 && head != NULL) {
    for (struct ifaddrs *p = head; p != NULL; p = p->ifa_next) {
      ...
      char ip[INET_ADDRSTRLEN] = {0};
      if (inet_ntop(AF_INET, &ga, ip, sizeof(ip)))
        gw = [[NSString stringWithUTF8String:ip] stringByAppendingString:@" (推定)"];
      break;
    }
    freeifaddrs(head);
  }
...
static int DBDeviceInfo(void *user, char **out_json) {
  ...
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];  // 非Cocoaスレッドから呼ばれる
...
- (void)stop {
  if (_diTimer) { [_diTimer invalidate]; [_diTimer release]; _diTimer = nil; }
...
- (NSString *)cachedDeviceInfoJson {
  [_diLock lock];
  NSString *s = [[_deviceInfoCache retain] autorelease];
  [_diLock unlock];
  return s;
}

// gateway / wifi(SSID,BSSID) / battery を今この場で取得して dict で返す。
// メインスレッド専用 (UIDevice/CaptiveNetwork は main 限定)。debug 画面が直接呼ぶ。
- (NSDictionary *)deviceInfoNow {
  ...
  if (ifs) {
    for (NSString *ifn in ifs) {
      NSDictionary *info = (NSDictionary *)CNCopyCurrentNetworkInfo((CFStringRef)ifn);
      if (info) {
