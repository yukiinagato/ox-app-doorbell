...
最终 `source-candidate-r7.json` 冻结整套输入；`owned-inputs-r7.json` 为本卡十个文件。`final-owned-input-check.json` 确认 canonical 十项与冻结构建均一致。构建使用主任务指定的 **T12 green Core**，仅叠加已授权的公告期限 `int64_t` 类型转换，避免混入并行 T28 事务开发。该类型转换也已由主任务修入 canonical；本卡未自行改 canonical Core。后续 Core 集成仍需重建。

## 已执行验证

- modern r7：完整 armeabi-v7a、arm64-v8a、x86_64 APK 及测试 APK，**386/386 JVM 测试、lint、构建通过**；Java21、NDK27.1.12297006。
- legacy19 r7：独立项目和 native cache，armeabi-v7a API19 APK，**386/386 JVM 测试、lint、构建通过**；NDK25.2.9519653。
- 实际 API35/Android15 arm64 模拟器：竖屏/横屏 × 字体1.0/2.0，**4组各8项，共32项通过**。每组保存截图、真实 View 树、instrumentation 原始日志、命令、APK hash 和来源标识。横屏与竖屏大字体的主要按钮、无人接听提示和 SOS 确认截图已人工查看，无裁切或重叠。
...
独立复核 R1 发现原 r6 的 AlertDialog positive 回调已排队后，cancelCountdown 不能撤销该回调。原 r6 APK 加新真实 UI 测试复现零秒倒计时触发1次（记录器），见 `red-r7-portrait-1.0`；原报告/验收和构建产物身份已保留为 r6 历史。r7 绑定 dialog 身份及 revision，取消、拆离、重新配置撤销旧确认，投递时重查 shown/windowToken；旧 dismiss 只能清理自身。四组各增加 cancel、detach 后同帧重挂、旧 A 回调与新 B 对话框三项实际 UI 用例。三种旧回调均零触发且不armed，新 B 的有效零秒确认恰好一次触发。

历史 r1 构建暴露公告期限 long long 的 Android 重载歧义，已由主任务确认修复。初期运行器在 Application 初始化前读取状态、等待持续动画 idle、以及在原生 AlertDialog 异步回调前立即断言，属于保留日志中的测试问题；最终改为初始化屏障、frame 同步与异步回调后断言，不将这些结果改写为产品缺陷 RED。r6 增加固定无人接听 strip 和生产 timeout 路径；r7 修正独立复核 R1，最终证据全部来自 r7，未用旧候选通过代替。

专用 AVD `doorbell-t35-api35` 最终验证结束后已停止，字体设置恢复1.0，测试 app 已 force-stop。标准 SDK runtime 与该独立 AVD 保留便于后续验收；没有操作物理设备、重启用户设备或删除其他任务产物。`acceptance.json` 是本子项验收索引；最终资格范围和未完成项以其明确限制为准。
