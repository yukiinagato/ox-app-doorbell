...
iOS compatibility アプリでは、端末名と door ID に `<model>-<role>-<8-hex-token>`
（例：`iPad-mini-door-a3f29c10`）を提案します。role は `door` または `indoor` で、
door ID は常に `door` を使用します。両方の候補で同じランダムな token を共有します。
設定確定前の role 変更は未編集の自動端末名だけを更新し、手入力は保持します。
確定済みの端末名と ID は更新・再起動後も変わりません。
...
```jsonc
{
  ...
  "display": {                                  // 表示・焼付対策 (全端末既定; devices.<id>.local.display で上書き)
    ...
    "screensaver_after_s": 120,                 // 旧待機時間。display.screensaver を参照
  ...
  "time": {
    ...
    // 独立時刻サービス。core は OS の時計を変更せず、SNTP で信頼できる時刻を測定し、
    // 単調時計アンカーから HLC、イベントと呼出履歴のタイムスタンプ、ルール、静音時間帯、
    // 画面の時計を算出する。測定値は次回の成功した同期か NTP の無効化まで維持する。
...
ワーカースレッドで実行する。サーバごとに 3 サンプル取り、往復時間が最小のものを採用する。通常は
往復 3 秒超またはオフセット 24 時間超のサンプルを捨てる。初回復旧では 30 日以内のずれを
往復時間の短い安定した応答 3 回で確認してから適用する。測定値は単調時計アンカーになり、OS 時計の
変更が補正値に重なることも補正を消すこともない。OS 時計は決して書き換えない。最後の信頼できる
アンカーはネットワークが切れても維持し、次の成功した定期同期で置き換えるか、NTP を無効化すると撤回する。
`POST /api/time/sync` (管理セッション) は即時の 1 回を開始する。結果は `status.time` に出て、
ソースが切り替わるか適用中のオフセットが 500 ms を超えて動いたとき `time_changed` を送出する。

HLC は物理時刻とは別に因果順序を保持する。未来の peer タイムスタンプや補正前の進んだ
ローカルタイムスタンプを保持していても、画面の時計、ローカルイベントの `wall_ms`、
スケジュール・期限の判定は進まない。NTP による後方補正も直ちに反映される。
HLC 識別子の単調性は維持され、複製された書き込みの順序は保たれる。
...
## 機器の削除、リソース同期、動体検知による画面復帰

認証済み管理セッションの `POST /api/devices/remove` に `{ "id": "<node-id>" }` を渡すと、対象の機器設定と映像再生の上書きを一括削除し、`removed_devices.<id>: true` を同期します。自分自身は削除できません。オフラインの記録はすぐに非表示になり、更新済みの対象端末は再接続時にペアリングを解除します。古い端末情報は一覧や通知担当の選出から除外されます。共有クラスタ鍵の更新を行う操作ではありません。

設定の削除は保存済みの子項目も対象です。削除記録は古い子項目と親オブジェクト内の値も隠すため、訪問目的をすべて削除した状態は同期後も維持されます。明示的な再作成は可能です。

各端末は参照中のリソースに加えて `assets` ライブラリ全体を取得し、保存前に SHA-256 を検証します。未取得のリソースは動作中に再試行します。完了まで取得元または完全な複製を持つ端末が接続可能である必要があります。

Telegram 設定には引き続き `bot_token_ref` だけを保存します。更新済み端末は認証・暗号化済みのメッシュ接続で現在の参照先トークンを要求し、プラットフォームの安全なストレージに保存します。設定、UI イベント、起動ファイルには値を含めません。更新時は新しい参照を使用し、取得元の更新済み端末が最低一台必要です。他の連携の資格情報の配布方式は従来どおりです。Telegram の担当にはローカルトークン、実測済み TLS・電源・時計、WAN の確認または `https://api.telegram.org/` への定期 HTTPS 検査の成功が必要です。この検査は Web Push の資格にはなりません。資格を失ったリーダーのリースは無効になります。接続中の非担当端末からのテストは担当へ転送され、応答の `request` を `GET /api/test/telegram/<request>` で確認できます。成功は送信キューへの受付を意味し、Telegram の配信完了ではありません。

`devices.<id>.local.motion.wake_screen` は省略可能な真偽値で、既定は `false` です。管理画面の機器編集で有効にすると、その玄関の新しい動体検知で画面を点灯しスクリーンセーバーを終了します。過去の同期イベントや別の玄関では復帰しません。iOS と Windows は設定済みの通常輝度に戻し、待機時間をリセットします。

modern iOS の門口機では `devices.<id>.local.visitor_layout` に `standard`（既定）、`left`、`right`、`edges` を指定できます。左右配置は反対側の背景を空け、edges は時計を上、操作を下に配置します。ネイティブ設定または Admin → Devices → Edit で選択でき、他のクライアントは無視します。背景とのコントラストが不足する文字には反対色の柔らかな影を描き、各ラベル直下の最暗部・最明部から文字色を選択します。

modern iOS の来訪者画面では言語行を用件ボタンの下に配置します。時計と日付は背景の測定範囲と自動文字色を共有し、日付を薄くしません。領域ごとの明示的な色設定は優先されます。


## 返信音声サービスと事前生成

`speech.provider` は `system`（既定）または `google` です。定型文のアップロード音声を優先し、
次に同期済みの生成 MP3、未取得時はシステム音声を使います。自由文はシステム音声のままです。
生成端末の Web 管理「定型返信 → 返信音声と音声キャッシュ」で Google、API キー、声を設定します。
キーは生成端末の安全なストレージに保存し、参照と音声だけを同期します。システム音声の試聴は
ブラウザーの声なので端末と異なる場合があります。Google の試聴は生成済みファイルを再生します。

`speech.generator_node` は唯一の生成端末、`speech.google_key_ref` はキー参照です。
`speech.voices` の `ja/en/zh` は既定で `ja-JP-Standard-A`、`en-US-Standard-A`、
`cmn-CN-Standard-A`。`speech.speaking_rate` は 0.25–4（既定 1）、`speech.auto_cache` は既定で有効です。
`speech.cache.<fingerprint>` は生成 MP3 の自動索引です。手動編集しないでください。

本文・言語・声・速度・形式が同じ要求は共用します。返信の編集・削除後に古い結果は適用しません。
一度に一要求、最大 256 翻訳、各 3,000 UTF-8 バイトまで処理します。失敗は手動再試行、内容変更、
または App 再起動で再試行できます。返信表示はクラウド生成を待ちません。音声は既存のハッシュ検証付き
同期を利用し、オフライン再生できます。認証済み `GET /api/tts` は状態を返し、生成端末の
`POST /api/tts/cache` は不足音声を生成・再試行します。自動生成が無効でも利用できます。
状態にはキーやサービスのエラー本文を含めません。

Google Cloud で Text-to-Speech API と課金を有効化し、この API に制限したキーを作成します。
Standard 音声は現在毎月 400 万文字の無料枠があり、超過は有料です。他の声は料金が異なります。
App は無料枠を保証しません。サービス側でクォータを設定してください。
[設定](https://docs.cloud.google.com/text-to-speech/docs/get-started)、
[料金](https://cloud.google.com/text-to-speech/pricing)を参照してください。

## 待機画面と Telegram の応答更新

管理 → テーマと画面 → 待機中の画面で `display.screensaver` を設定します。端末ごとの `devices.<id>.local.display.screensaver` は項目単位で優先され、このオブジェクトを削除すると全体設定を継承します。

```json
{"enabled":true,"schedule":"daily","from":"22:00","to":"06:00","after_s":120,"brightness":20,"mode":"dim"}
```

`enabled` の既定値は true（旧待機時間がゼロの場合は false）。`schedule` は `always`（既定）または `daily` です。指定時間帯はクラスタのタイムゾーンで開始を含み終了を含まず、日付をまたげます。同じ開始・終了時刻は空の時間帯になるため、終日は `always` を使います。Core は最大30秒間隔で適用可否を更新します。`after_s` は1～86400の整数で、未設定時は旧 `screensaver_after_s`（120秒）を使用します。`brightness` は1～100の整数、既定値10で、通常時の明るさを超えません。無効化または時間帯終了の通知を受けた端末は通常表示へ戻ります。

現行 iOS と Windows の表示モードは `dim`（既定：全操作画面のまま減光）、`minimal`（壁紙を隠し操作部を残し、単色背景に合わせて文字色を再計算）、`clock`（黒背景・時計・日付・タッチでドアホン画面へ戻る案内）です。タッチ、着信、緊急時は通常表示に戻ります。Android、tvOS、iOS 5 は従来の表示動作を維持し、待機モードへの対応を宣言しません。実行時の `display.screensaver` は解決済みの項目と `eligible` を返し、時間帯外の旧 `screensaver_after_s` はゼロになります。

室内機の有効な `call_answered` または呼出しに紐づく `reply` に対し、Telegram 担当ノードは元の来客通知を編集し、来客情報に応答端末名と時刻（クイック返信なら本文も）を追加して古い返信ボタンを削除します。受理された同じ `call_id` のイベントだけが対象で、次の来客の通知は変更しません。写真送信完了前の応答も永続再試行キューで更新します。Telegram からの返信は既存のコールバック処理を維持します。HTTPS モックで検証し、実際の家庭用チャットには試験メッセージを送信しません。
