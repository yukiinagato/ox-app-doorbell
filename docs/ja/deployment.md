...
- [ ] Windows: GitHub Actions の `doorbell-windows` artifact を展開 →
      ...
      `kiosk-enable.cmd` → 再ログイン (詳細: `docs/win-build-env.md` §実機)
...
- [ ] 呼出ルール: 按鈴 → SIP 600 + Telegram + チャイム。宅配 (p_delivery) だけ
...
- Android: `deploy/provision/android/provision.md` (Device Owner 化 = 完全 kiosk、TV 節あり)
- iOS: `deploy/provision/ios/provision.md` (監督 + Single App Mode、Ad Hoc 署名と年次更新)
...
- [ ] Mosquitto へ接続された時点で HA に実体が自動出現 (呼び鈴 event / 動体 /
      端末オンライン / 緊急 / 訪客言語 sensor)
- [ ] `deploy/ha/go2rtc.yaml` を各門口機の IP に合わせて取り込み
      (codec=h264 の端末は `#video=copy` — 転码不要)
- [ ] `deploy/ha/configuration-snippets.yaml` の HomeKit Bridge / 看門狗 /
      actionable 通知 / 開錠 automation を取り込み、entity_id を実物に合わせる
- [ ] iPhone の家庭 App に門鈴通知+ライブが出ることを確認。外出先から見るには
      Apple TV / HomePod をホームハブに

## 5. 動作検証 (端末追加のたび)

- [ ] 按鈴 → 内線+携帯が鳴る / Telegram 写真+ボタン / HA 通知 / 室内チャイム
- [ ] Telegram のボタンで返信 → 門口機に大字+読み上げ
- [ ] 室内機から「応答」→ 電話腿が切れて双方向通話 (映像含む)
- [ ] 通話中に携帯から *1 → HA の錠が開く
- [ ] 端末の LAN ケーブルを抜く → 30 秒以内に Telegram/HA へオフライン通知
- [ ] **HA と Asterisk を止めても**: 按鈴表示・チャイム・室内対講・パネルが動き続ける

## 6. 運用

- 更新: GitHub Actions の artifact を配布 (Windows は watchdog が停止→差替→再起動を
  許容。Android は DO 静默インストール)。更新前にタグを打つと戻しやすい
- iOS Ad Hoc 署名は **年 1 回の再署名が必須** — アプリ内の期限表示と Telegram の
  30 日前警告に従う (恒久化は App Store 上架 = 計画 Phase 7)
- 設定バックアップ: 管理画面「システム」→ エクスポート (どのノードでも全量)
- 端末盗難時: 管理画面で PSK 再発行 → 全端末再配対、SIP パスワード・bot token を回転
