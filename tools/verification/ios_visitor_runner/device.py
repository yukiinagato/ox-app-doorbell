...
p=argparse.ArgumentParser();p.add_argument('action',choices=['install','wait','collect','restore','status']);p.add_argument('--lane',choices=['baseline','candidate'],default='candidate');a=p.parse_args();r=pathlib.Path.cwd();b=r/('build/remediation-t35-modern-device-'+a.lane);out=r/'verification/remediation-q01-q18/T35-ios-modern/evidence/device-uikit';host='doorbell-ipad-air1';remote='/private/var/containers/Bundle/Application/F3545678-1234-4234-8234-123456789ABC';owned='/var/mobile/Library/DoorbellT35ModernUIKit'
...
if a.action=='install':
 ...
 ssh(owned+'/register inspect '+remote+'/DoorbellIOS9.app/Info.plist; /binpack/usr/local/bin/lsdtrip.arm64 launch jp.ox.doorbell.t35modernuitest')
elif a.action=='wait':
 ...
 destination=out/('air1-results-'+a.lane);destination.mkdir(exist_ok=True)
 ssh('ls -l '+owned)
 ...
 result=out/('air1-results-'+a.lane)/'result.json'
 if result.exists():
